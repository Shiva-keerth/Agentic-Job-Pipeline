"""
Round 5: Scrape LinkedIn, evaluate via LLM, return 10 NEW qualified jobs.
- STRICT: Only jobs posted in last 48 hours (r172800).
- Excludes all jobs from Rounds 1, 2, 3, and 4.
- Uses fresh keyword variations to avoid overlap.
"""
...
# Fresh keywords for Round 5
SEARCH_KEYWORDS = [
    "AI Engineer fresher 2026",
    "Generative AI developer entry level",
    "LLM Engineer junior",
    "Python AI developer 0 experience",
    "RAG engineer fresher",
    "Agentic AI developer",
    "AI ML engineer trainee",
    "NLP engineer fresher",
    "Machine learning engineer 2026 graduate",
    "AI solutions engineer India",
    "LangChain developer fresher",
    "Large Language Model engineer junior",
]
...
def scrape_search_page(keyword: str, start: int = 0) -> list:
    """Scrape LinkedIn search results — STRICTLY last 48 hours (r172800)."""
    url = (
        ...
        f"&f_E=2"
        f"&f_JT=F"
        f"&f_TPR=r172800"  # 48 hours = 172800 seconds
        f"&start={start}"
    )
    try:
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  JOBLINE PIPELINE — ROUND 5 (48-Hour Fresh Jobs Only)")
    print(f"  Excluding {len(EXCLUDE_URLS)} previous jobs + {len(EXCLUDE_COMPANIES)} companies")
    print(f"  Searching past 48 HOURS ONLY across {len(SEARCH_KEYWORDS)} keywords")
    print(f"{'='*60}\n")
    ...
    print(f"\n\n{'='*60}")
    print(f"  FINAL RESULTS: {len(valid_jobs)} Qualified Jobs (Round 5 — 48h)")
    print(f"{'='*60}\n")
    ...
    output_file = "top10_round5_results.json"
