"""
Round 29: AI Engineer / GenAI / LLM ONLY — 15 Results Target
- 24-HOUR Window, NO entry-level filter (experience pre-filter handles it)
- Excludes ALL 28 previous rounds
- Fresh keyword angles: fresher-specific, 0-1 YOE, entry-level, junior, associate
- Blocks prompt engineering roles
- Targets: fresher, entry-level, 0-1 year, trainee, associate, junior
"""
...
BLOCKED_ROLE_KEYWORDS = [
    ...
    "consultant", "architect", "lead", "director", "principal", "manager"
]
...
# ── Load ALL previous rounds (1 through 29) to exclude URLs and roles ──
prev_files = [
    "top10_results.json",
    *[f"top10_round{i}_results.json" for i in range(2, 30)]
]
...
print(f"Loaded {len(EXCLUDE_URLS)} previous URLs and {len(PREV_SEEN_KEYS)} previous roles to exclude.")

# ── Round 29: FRESH keyword angles — fresher/entry-level focused ──
SEARCH_KEYWORDS = [
    "AI Engineer fresher 0 years India 2026",
    "GenAI Engineer fresher entry level India",
    "LLM Engineer entry level India 2026",
    "Generative AI fresher India Bangalore",
    "AI Engineer 0 1 year experience India",
    "Junior AI Developer Python LangChain India",
    "Entry level Machine Learning Engineer GenAI India",
    "Associate AI Engineer LLM India 2026",
    "Fresher AI ML Python Engineer India Hyderabad",
    "Junior LLM Developer LangGraph FastAPI India",
    "Trainee AI Engineer GenAI India Mumbai",
    "Entry level NLP Engineer LLM India",
    "Junior Generative AI Engineer Python India Pune",
    "AI Associate Engineer RAG LangChain India",
    "Fresher Python AI Engineer Groq OpenAI India",
    "GenAI Developer 0 experience India remote",
    "AI Software Engineer fresher 2026 India",
    "Junior RAG Engineer Python India",
]
...
def main():
    ...
    print(f"{'='*62}")
    print(f"  JOBLINE PIPELINE — ROUND 30 (Target: {TARGET} Results)")
    print(f"  24-HOUR Window | Experience pre-filter | AI/GenAI/LLM ONLY")
    print(f"  Focus: FRESHER / ENTRY-LEVEL / 0-1 YOE roles")
    ...
    output_file = "top10_round30_results.json"
    ...
    print("\n[DB & Email Automation]")
    os.system(f"python save_round_to_db.py {output_file}")
    os.system(f"python notify_email.py {output_file} --min-score 75")
