"""
Round 3: Scrape LinkedIn, evaluate via LLM, return 10 NEW qualified jobs.
- Excludes all jobs from BOTH previous rounds
- Automatically excludes companies from previous rounds to guarantee variety
- Searches pages deeper into the results (25, 50, 75)
"""
...
prev_files = ["top10_results.json", "top10_round2_results.json"]
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  JOBLINE PIPELINE — ROUND 3 (Fresh {TARGET} Jobs)")
    ...
    print(f"\n\n{'='*60}")
    print(f"  FINAL RESULTS: {len(valid_jobs)} Qualified Jobs (Round 3)")
    print(f"{'='*60}\n")
    ...
    with open("top10_round3_results.json", "w", encoding="utf-8") as f:
        json.dump(valid_jobs, f, indent=2, ensure_ascii=False)
    print(f"\nResults saved to top10_round3_results.json")
