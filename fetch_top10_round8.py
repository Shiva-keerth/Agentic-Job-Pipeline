"""
Round 8: Scrape LinkedIn, evaluate via LLM, return 10 NEW qualified jobs.
- Excludes ALL jobs from Rounds 1-7.
- Uses completely fresh keyword variations.
- STRICT 2-day window (r172800) for most recent openings only.
...
# Fresh keywords for Round 8 — completely new set
SEARCH_KEYWORDS = [
    "machine learning engineer fresher 2026 India",
    "AI research assistant entry level India",
    "data science fresher generative AI India",
    "LLM developer associate Bengaluru 2026",
    "AI backend engineer junior Mumbai",
    "natural language processing developer fresher",
    "AI solutions engineer trainee India",
    "Groq llama developer fresher India",
    "multimodal AI developer entry level",
    "AI agent developer fresher Delhi NCR",
    "applied AI engineer 0 to 1 year India",
    "full stack AI developer fresher India",
]
...
def scrape_search_page(keyword: str, start: int = 0) -> list:
    ...
    url = (
        ...
        f"&f_E=2"
        f"&f_JT=F"
        f"&f_TPR=r172800"   # 2 days = 172800 seconds
        f"&start={start}"
    )
    try:
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  JOBLINE PIPELINE — ROUND 8 (2-Day Window)")
    ...
    print(f"\n\n{'='*60}")
    print(f"  FINAL RESULTS: {len(valid_jobs)} Qualified Jobs (Round 8 — 2-day)")
    print(f"{'='*60}\n")
    ...
    output_file = "top10_round8_results.json"
