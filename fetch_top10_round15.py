"""
Round 15: AI Engineer / GenAI pipeline — 15 Results Target
- All previous rounds 1-14 excluded
- Completely fresh keywords
- STRICT experience filter: rejects roles requiring 2+ years
- Target: 15 accurate results
"""
...
# ── Exclusion list: ALL previous rounds 1-14 ──
EXCLUDE_COMPANIES = {
    ...
    "hire feed", "mccormick", "gartner", "bayone solutions",
    "amazon", "spg consulting", "nielsen", "brainware university",
    ...
    # Round 14
    "meraki labs", "hrk global techsolutions pvt. ltd.", "hackerrank",
    "turing", "equal", "stance health", "tasks expert",
    "callkaro.ai", "iitmatrix", "setoo",
}

prev_files = [
    f"top10_results.json", "top10_round2_results.json", "top10_round3_results.json",
    ...
    "top10_round13_results.json", "top10_round14_results.json",
]
...
# ── Round 15 FRESH keywords ──
SEARCH_KEYWORDS = [
    "conversational AI developer fresher India",
    "agentic AI Python developer India 2026",
    "NLP developer fresher Bangalore",
    "LangGraph developer junior India",
    "AI chatbot developer India fresher",
    "OpenAI API developer junior India",
    "vector database developer fresher India",
    "knowledge graph developer junior India",
    "Groq LLM developer India",
    "AI SaaS developer junior India",
    "multimodal AI developer fresher India",
    "AI startup engineer fresher India",
    "retrieval augmented generation developer India",
    "Hugging Face developer junior India",
    "voice AI developer fresher India",
]
...
def is_role_allowed(title: str) -> tuple:
    ...
    return False, "Role not in AI Engineer whitelist"
...
def analyze_apply_decision(missing_skills: list) -> tuple:
    ...
    return "APPLY", "Missing skills appear to be optional tools"
...
def fetch_jd_text(job_url: str) -> tuple:
    ...
    try:
        ...
        if not jd or len(jd) < 80:
            for selector in ["div.description__text", "section.show-more-less-html", "div[class*='description']"]:
                try:
                    el = soup.select_one(selector)
                    if el:
...
def main():
    ...
    print(f"{'='*62}")
    print(f"  JOBLINE PIPELINE — ROUND 15 (Target: 15 Results)")
    print(f"  2-Day Window | LinkedIn Public Search")
    print(f"  Excluding {len(EXCLUDE_URLS)} URLs + {len(EXCLUDE_COMPANIES)} companies")
    ...
    for kw_idx, keyword in enumerate(SEARCH_KEYWORDS):
        ...
        for card in cards:
            ...
            if not role_ok:
                ...
                print(f"    [ROLE] Rejected '{safe_title}' — {role_reason}")
                continue
    ...
    print(f"  Easy Apply: {sum(1 for j in valid_jobs if not j['is_external_apply'])}")
    print(f"  External:   {sum(1 for j in valid_jobs if j['is_external_apply'])}")
    print(f"  Role filter rejected: {role_rejected_count}")
    print(f"{'='*62}\n")
    ...
    for i, job in enumerate(valid_jobs, 1):
        ...
        print(f"  Skills OK:  {', '.join(job['matched_skills'][:8]) or 'N/A'}")
        print(f"  Missing:    {', '.join(job['missing_skills'][:5]) or 'None'}")
        print(f"  LINK:       {job['url']}")
        print()

    output_file = "top10_round15_results.json"
