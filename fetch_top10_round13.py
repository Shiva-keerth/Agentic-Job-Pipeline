"""
Round 13: AI Engineer / GenAI pipeline — External ATS Support Added.
- NEW: fetch_jd_text now follows external apply links (Workday, Greenhouse, Lever, Naukri, etc.)
- Excludes ALL jobs from Rounds 1-12.
- Completely fresh search keywords.
...
# ── Exclusion list: ALL previous rounds 1-12 ──
EXCLUDE_URLS = set()
EXCLUDE_COMPANIES = {
    # Big IT — always block
...
# ── Round 13 — FRESH keywords, different angle ──
SEARCH_KEYWORDS = [
    "AI engineer startup India 0-1 year 2026",
    "LLM backend engineer fresher Bengaluru",
    "GenAI Python developer entry level Chennai",
    "agentic AI developer fresher Hyderabad 2026",
    "RAG pipeline developer junior Delhi",
    "AI product developer India immediate joiner",
    "machine learning Python fresher Noida Gurgaon",
    "NLP developer junior India 2026",
    "AI chatbot engineer entry level India",
    "LangChain developer fresher Bengaluru 2026",
    "AI inference engineer junior India",
    "Python backend AI developer fresher India 2026",
    "knowledge graph engineer junior India",
    "AI API developer fresher India 2026",
    "junior software engineer AI startup India 2026",
]
...
# ATS-specific CSS selectors ordered by specificity
ATS_SELECTORS = [
    # Greenhouse
...
def fetch_external_jd(url: str) -> str:
    """
    Fetch JD text from an external company career page.
    Tries ATS-specific selectors first, then falls back to the
    largest meaningful text block on the page.
    """
    try:
        ...
        # Fallback: find the largest <div>/<section> block (cap at 8000 chars to avoid full-page dumps)
...
def fetch_jd_text(job_url: str) -> tuple:
    """
    Returns (jd_text, is_external_apply).
    is_external_apply = True when we had to follow an external URL to get the JD.

    Strategy:
      1. Fetch LinkedIn job page.
      2. Try LinkedIn-hosted JD selector (Easy Apply jobs).
      3. If empty, try broader LinkedIn selectors.
      4. If still empty, extract external apply URL and fetch from there.
    """
    is_external = False
    try:
        ...
        # Remove sidebar noise
        for sidebar in soup.find_all(class_=["similar-jobs", "people-also-viewed"]):
            sidebar.decompose()

        # ── Step 1: LinkedIn-hosted JD (Easy Apply) ──
        desc = soup.find("div", class_="show-more-less-html__markup")
        jd = desc.get_text(separator=" ", strip=True) if desc else ""

        # ── Step 2: Broader LinkedIn selectors (some external jobs still embed preview JD) ──
        ...
        # ── Step 3: Follow external apply URL if JD is still empty/short ──
        if not jd or len(jd) < 80:
            external_url = None

            # Method A: look for serialized applyUrl in page source
            src = r.text
            match = re.search(r'"applyUrl"\s*:\s*"(https?://[^"]+)"', src)
            if match:
                external_url = match.group(1).replace("\\u003d", "=").replace("\\u0026", "&")

            # Method B: look for apply button href pointing outside LinkedIn
            if not external_url:
            ...
            # Method C: find any external link with apply/job/career in it
            if not external_url:
                for a_tag in soup.find_all("a", href=True):
                    href = a_tag.get("href", "")
                    if "linkedin.com" not in href and href.startswith("http"):
        ...
        # Add LinkedIn poster requirements block (if available)
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  JOBLINE PIPELINE — ROUND 13 (External ATS Support)")
    ...
    output_file = "top10_round13_results.json"
