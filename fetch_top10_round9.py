"""
Round 9: AI Engineer / GenAI ONLY pipeline.
- Strict role-title WHITELIST: Only AI Engineer, GenAI, LLM, Agentic, Prompt Engineer roles.
- Strict role-title BLACKLIST: Rejects Data Scientist, Research Analyst, Technical Writer, etc.
- Excludes ALL jobs from Rounds 1-8.
- Focused AI Engineer keywords only.
- STRICT 2-day window (r172800).
- "Apply or Skip?" analysis for each job based on missing skill type.
"""
...
# ── ROLE TITLE WHITELIST: Only these types of roles are allowed ──
...
# ── ROLE TITLE BLACKLIST: These roles are immediately rejected ──
...
TOOL_GAP_SIGNALS = [
    ...
    "celery", "redis", "kafka", "kubernetes", "terraform",
    "mlflow", "wandb", "weights & biases", "dvc",
    "node.js", "react", "angular", "vue",
    "snowflake", "databricks", "spark", "hadoop",
...
CORE_GAP_SIGNALS = [
    ...
    "salesforce", "sap", "erp",
    "itsm", "servicenow", "infrastructure monitoring",
    "sampling analytics",
]

def is_role_allowed(title: str) -> tuple[bool, str]:
    """Returns (True, reason) if role title is allowed, else (False, reason)."""
    ...
    # If not in either list, reject by default (too ambiguous)
    return False, "Role title not in AI Engineer whitelist"


def analyze_apply_decision(missing_skills: list) -> tuple[str, str]:
    """
    Returns ("APPLY", reason) or ("SKIP", reason) based on missing skill type.
    Tool gaps = APPLY, Core skill gaps = SKIP.
    """
    if not missing_skills:
        return "APPLY", "No missing skills — perfect match!"

    missing_lower = [s.lower() for s in missing_skills]
    missing_text = " ".join(missing_lower)

    core_gaps_found = []
    for signal in CORE_GAP_SIGNALS:
        if signal in missing_text:
            core_gaps_found.append(signal)

    tool_gaps_found = []
    for signal in TOOL_GAP_SIGNALS:
        if signal in missing_text:
            tool_gaps_found.append(signal)
    ...
    if core_gaps_found:
        ...
        # Missing skills not in either list — assume tool gaps for GenAI roles
        return "APPLY", f"Missing skills appear to be optional tools, not core blockers"


# ── Exclusion list: all URLs and Companies from ALL previous rounds ──
EXCLUDE_URLS = set()
EXCLUDE_COMPANIES = {
    # Big IT service companies — always block
...
prev_files = [
    ...
    "top10_round6_results.json",
    "top10_round7_results.json",
    "top10_round8_results.json",
]
...
# Focused AI Engineer / GenAI keywords ONLY
SEARCH_KEYWORDS = [
    "GenAI engineer fresher India 2026",
    "LangChain LangGraph developer entry level India",
    "agentic AI developer junior India",
    "RAG pipeline engineer fresher Bengaluru",
    "LLM developer junior India 2026",
    "AI backend engineer fresher Python FastAPI",
    "multi-agent AI developer entry level India",
    "prompt engineer associate India 2026",
    "AI software engineer fresher Hyderabad",
    "generative AI engineer junior Pune",
    "LLM application engineer 0 to 1 year",
    "AI engineer fresher immediate joiner India",
]
...
def scrape_search_page(keyword: str, start: int = 0) -> list:
    """Scrape LinkedIn search results — last 2 days only (r172800)."""
    url = (
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  JOBLINE PIPELINE — ROUND 9 (AI Engineer / GenAI ONLY)")
    print(f"  2-Day Window | Strict Role Title Filtering")
    print(f"  Excluding {len(EXCLUDE_URLS)} previous jobs + {len(EXCLUDE_COMPANIES)} companies")
    print(f"  Searching across {len(SEARCH_KEYWORDS)} focused AI keywords")
    print(f"{'='*60}\n")
    ...
    for kw_idx, keyword in enumerate(SEARCH_KEYWORDS):
        ...
        for card in cards:
            ...
            # ── NEW: Strict role title check BEFORE fetching JD ──
    ...
    output_file = "top10_round9_results.json"
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(valid_jobs, f, indent=2, ensure_ascii=False)
    print(f"\nResults saved to {output_file}")
    print(f"\nSUMMARY: {sum(1 for j in valid_jobs if j['apply_decision'] == 'APPLY')} jobs to APPLY | {sum(1 for j in valid_jobs if j['apply_decision'] == 'SKIP')} jobs to SKIP")
