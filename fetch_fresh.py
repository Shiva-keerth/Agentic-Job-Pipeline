...
# Get all applied/evaluated jobs
cursor.execute("SELECT company, role FROM evaluator_telemetry")
seen = {(row[0].lower(), row[1].lower()) for row in cursor.fetchall()}
try:
    cursor.execute("SELECT company, role FROM applications")
    seen.update({(row[0].lower(), row[1].lower()) for row in cursor.fetchall()})
except:
    pass
...
for card in cards:
    try:
        ...
        if (company.lower(), title.lower()) in seen:
            continue
