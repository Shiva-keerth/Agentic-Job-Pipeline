"""
One-shot script: Scrape LinkedIn, evaluate via LLM, return exactly 10 qualified jobs with links.
Searches past 7 days (not just 24h) across multiple keywords for maximum coverage.
"""
...
def fetch_jd_text(job_url: str) -> str:
    """Fetch the full JD text from a LinkedIn job page."""
    try:
...
def scrape_search_page(keyword: str, time_range: str = "r604800", start: int = 0) -> list[dict]:
    """Scrape one page of LinkedIn search results. Returns list of {title, company, link}."""
    url = (
        ...
        f"&f_TPR={time_range}"  # Past 7 days
        f"&start={start}"
    )
    try:
...
def main():
    ...
    seen_keys = set()       # Dedup by company+role

    print(f"{'='*60}")
    print(f"  JOBLINE PIPELINE — Finding Top {TARGET} Jobs For You")
    ...
    for kw_idx, keyword in enumerate(SEARCH_KEYWORDS):
        ...
        # Scrape page 0 and page 1 (up to 50 cards per keyword)
        cards = []
        ...
        for card in cards:
            ...
            if link in seen_urls or dedup_key in seen_keys:
                continue
            seen_urls.add(link)
            ...
            if not jd_text:
                print(f"      -> Skipped (empty JD or closed)")
                continue
        ...
        # Breathing room between keywords
        time.sleep(random.uniform(3, 6))

    # ── FINAL OUTPUT ──
    print(f"\n\n{'='*60}")
    print(f"  FINAL RESULTS: {len(valid_jobs)} Qualified Jobs")
    print(f"{'='*60}\n")
    ...
    for i, job in enumerate(valid_jobs, 1):
        ...
        print(f"  Skills OK:  {', '.join(job['matched_skills'][:8])}")
        print(f"  Missing:    {', '.join(job['missing_skills'][:5]) if job['missing_skills'] else 'None'}")
        print(f"  LINK:       {job['url']}")
        print()

    # Also dump as JSON for easy parsing
    with open("top10_results.json", "w", encoding="utf-8") as f:
        json.dump(valid_jobs, f, indent=2, ensure_ascii=False)
    print(f"\nResults also saved to top10_results.json")
